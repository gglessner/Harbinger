    ...
    def send_email(self, to_address, subject, body):
        ...
        if email_config.get('use_local_mail', False):
            try:
                ...
                # Use list format for better escaping, avoiding shell=True for security
                # Basic format: mail -s subject recipient
                cmd_parts = [mail_cmd, '-s', subject, to_address]
                
                logging.info(f"Executing mail command: {cmd_parts}")
                ...
                if process.returncode == 0:
                    ...
                    logging.error(f"Failed to send local mail to {to_address}")
