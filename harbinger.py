...
class HarbingerMonitor:
    ...
    def generate_report(self, email_address, port_data):
        ...
        if total_errors > 0 and total_new_hosts == 0:
            ...
            subject = f"Harbinger Report: {total_new_hosts} new hosts detected, {total_errors} scan failures"
        elif total_new_hosts > 0:
            subject = f"Harbinger Report: {total_new_hosts} new host{'s' if total_new_hosts != 1 else ''} detected"
        else:
            subject = "Harbinger Report: No new hosts detected"
...
"""
        ...
        self.save_report_to_file(email_address, subject, body)
    ...
    def save_report_to_file(self, email_address, subject, body):
        ...
        try:
            ...
            filename = f"{reports_dir}/{username}_{timestamp}.txt"
...
"""
    ...
    def scan_ports(self):
        ...
        for section_name, section_data in self.config.items():
            ...
            if not port or not email:
                logging.warning(f"Invalid port configuration in {section_name}: missing port or email")
                continue
            
            # Initialize email group if not exists
            ...
            try:
                if nmap_scan:
                    ...
                    email_groups[email][port] = {'error': scan_error, 'hosts': []}
                    continue
            except Exception as e:
                ...
                email_groups[email][port] = {'hosts': [], 'error': scan_error}
                continue
            
            # Update database and collect new hosts (only if scan was successful)
            if scan_success:
                new_hosts = self.update_host_database(port, ip_addresses)
                email_groups[email][port] = {'hosts': new_hosts, 'error': None}
            else:
                # Store scan failure information
                email_groups[email][port] = {'hosts': [], 'error': scan_error}
        
        # Generate reports for each email address
        for email_address, port_data in email_groups.items():
            self.generate_report(email_address, port_data)
