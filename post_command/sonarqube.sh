#!/bin/bash
# SonarQube Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full SonarQube security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. SonarQube security testing
#
# Always returns 0 for successful chain execution, regardless of final sonarqube.py result
...
echo "=== SonarQube Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe sonarqube.py can still work
    fi
    
    # Step 4: SonarQube TLS testing
    echo ""
    echo "Step 4: SonarQube TLS security testing..."
    if python3 post_command/sonarqube.py --tls "$HOST" "$PORT"; then
        echo "SonarQube TLS connection successful"
    else
        echo "SonarQube TLS connection failed (may not be a SonarQube service)"
    fi
else
    # Step 4: SonarQube plain testing
    echo ""
    echo "Step 4: SonarQube plain connection testing..."
    if python3 post_command/sonarqube.py "$HOST" "$PORT"; then
        echo "SonarQube plain connection successful"
    else
        echo "SonarQube plain connection failed (may not be a SonarQube service)"
    fi
fi

echo ""
echo "=== SonarQube Security Scan Chain Complete ==="
