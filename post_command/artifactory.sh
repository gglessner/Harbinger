#!/bin/bash
# Artifactory Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Artifactory security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Artifactory security testing
#
# Always returns 0 for successful chain execution, regardless of final artifactory.py result
...
echo "=== Artifactory Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe artifactory.py can still work
    fi
    
    # Step 4: Artifactory TLS testing
    echo ""
    echo "Step 4: Artifactory TLS security testing..."
    if python3 post_command/artifactory.py --tls "$HOST" "$PORT"; then
        echo "Artifactory TLS connection successful"
    else
        echo "Artifactory TLS connection failed (may not be an Artifactory service)"
    fi
else
    # Step 4: Artifactory plain testing
    echo ""
    echo "Step 4: Artifactory plain connection testing..."
    if python3 post_command/artifactory.py "$HOST" "$PORT"; then
        echo "Artifactory plain connection successful"
    else
        echo "Artifactory plain connection failed (may not be an Artifactory service)"
    fi
fi

echo ""
echo "=== Artifactory Security Scan Chain Complete ==="
