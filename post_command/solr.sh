#!/bin/bash
# Apache Solr Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Apache Solr security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Apache Solr security testing
#
# Always returns 0 for successful chain execution, regardless of final solr.py result
...
echo "=== Apache Solr Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe solr.py can still work
    fi
    
    # Step 4: Apache Solr TLS testing
    echo ""
    echo "Step 4: Apache Solr TLS security testing..."
    if python3 post_command/solr.py --tls "$HOST" "$PORT"; then
        echo "✓ Apache Solr TLS connection successful"
    else
        echo "✗ Apache Solr TLS connection failed (may not be a Solr service)"
    fi
else
    # Step 4: Apache Solr plain testing
    echo ""
    echo "Step 4: Apache Solr plain connection testing..."
    if python3 post_command/solr.py "$HOST" "$PORT"; then
        echo "✓ Apache Solr plain connection successful"
    else
        echo "✗ Apache Solr plain connection failed (may not be a Solr service)"
    fi
fi

echo ""
echo "=== Apache Solr Security Scan Chain Complete ==="
