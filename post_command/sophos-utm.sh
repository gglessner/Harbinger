#!/bin/bash
# Sophos UTM Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Sophos UTM security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Sophos UTM security testing
#
# Always returns 0 for successful chain execution, regardless of final sophos-utm.py result
...
echo "=== Sophos UTM Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe sophos-utm.py can still work
    fi
    
    # Step 4: Sophos UTM TLS testing
    echo ""
    echo "Step 4: Sophos UTM TLS security testing..."
    if python3 post_command/sophos-utm.py --tls "$HOST" "$PORT"; then
        echo "Sophos UTM TLS connection successful"
    else
        echo "Sophos UTM TLS connection failed (may not be a Sophos UTM service)"
    fi
else
    # Step 4: Sophos UTM plain testing
    echo ""
    echo "Step 4: Sophos UTM plain connection testing..."
    if python3 post_command/sophos-utm.py "$HOST" "$PORT"; then
        echo "Sophos UTM plain connection successful"
    else
        echo "Sophos UTM plain connection failed (may not be a Sophos UTM service)"
    fi
fi

echo ""
echo "=== Sophos UTM Security Scan Chain Complete ==="
