#!/bin/bash
# Redis Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Redis security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Redis security testing
#
# Always returns 0 for successful chain execution, regardless of final redis.py result
...
echo "=== Redis Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe redis.py can still work
    fi
    
    # Step 4: Redis TLS testing
    echo ""
    echo "Step 4: Redis TLS security testing..."
    if python3 post_command/redis.py --tls "$HOST" "$PORT"; then
        echo "Redis TLS connection successful"
    else
        echo "Redis TLS connection failed (may not be a Redis service)"
    fi
else
    # Step 4: Redis plain testing
    echo ""
    echo "Step 4: Redis plain connection testing..."
    if python3 post_command/redis.py "$HOST" "$PORT"; then
        echo "Redis plain connection successful"
    else
        echo "Redis plain connection failed (may not be a Redis service)"
    fi
fi

echo ""
echo "=== Redis Security Scan Chain Complete ==="
