#!/bin/bash
# TensorBoard Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full TensorBoard security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. TensorBoard security testing
#
# Always returns 0 for successful chain execution, regardless of final tensorboard.py result
...
echo "=== TensorBoard Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe tensorboard.py can still work
    fi
    
    # Step 4: TensorBoard TLS testing
    echo ""
    echo "Step 4: TensorBoard TLS security testing..."
    if python3 post_command/tensorboard.py --tls "$HOST" "$PORT"; then
        echo "TensorBoard TLS connection successful"
    else
        echo "TensorBoard TLS connection failed (may not be a TensorBoard service)"
    fi
else
    # Step 4: TensorBoard plain testing
    echo ""
    echo "Step 4: TensorBoard plain connection testing..."
    if python3 post_command/tensorboard.py "$HOST" "$PORT"; then
        echo "TensorBoard plain connection successful"
    else
        echo "TensorBoard plain connection failed (may not be a TensorBoard service)"
    fi
fi

echo ""
echo "=== TensorBoard Security Scan Chain Complete ==="
