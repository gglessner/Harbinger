#!/bin/bash
# Rundeck Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Rundeck security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Rundeck security testing
#
# Always returns 0 for successful chain execution, regardless of final rundeck.py result
...
echo "=== Rundeck Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe rundeck.py can still work
    fi
    
    # Step 4: Rundeck TLS testing
    echo ""
    echo "Step 4: Rundeck TLS security testing..."
    if python3 post_command/rundeck.py --tls "$HOST" "$PORT"; then
        echo "Rundeck TLS connection successful"
    else
        echo "Rundeck TLS connection failed (may not be a Rundeck service)"
    fi
else
    # Step 4: Rundeck plain testing
    echo ""
    echo "Step 4: Rundeck plain connection testing..."
    if python3 post_command/rundeck.py "$HOST" "$PORT"; then
        echo "Rundeck plain connection successful"
    else
        echo "Rundeck plain connection failed (may not be a Rundeck service)"
    fi
fi

echo ""
echo "=== Rundeck Security Scan Chain Complete ==="
