#!/bin/bash
# STOMP Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full STOMP security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. STOMP security testing
#
# Always returns 0 for successful chain execution, regardless of final stomp.py result
...
echo "=== STOMP Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe stomp.py can still work
    fi
    
    # Step 4: STOMP TLS testing
    echo ""
    echo "Step 4: STOMP TLS security testing..."
    if python3 post_command/stomp.py --tls "$HOST" "$PORT"; then
        echo "STOMP TLS connection successful"
    else
        echo "STOMP TLS connection failed (may not be a STOMP service)"
    fi
else
    # Step 4: STOMP plain testing
    echo ""
    echo "Step 4: STOMP plain connection testing..."
    if python3 post_command/stomp.py "$HOST" "$PORT"; then
        echo "STOMP plain connection successful"
    else
        echo "STOMP plain connection failed (may not be a STOMP service)"
    fi
fi

echo ""
echo "=== STOMP Security Scan Chain Complete ==="
