#!/bin/bash
# Apache Drill Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Apache Drill security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Apache Drill security testing
#
# Always returns 0 for successful chain execution, regardless of final drill.py result
...
echo "=== Apache Drill Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe drill.py can still work
    fi
    
    # Step 4: Apache Drill TLS testing
    echo ""
    echo "Step 4: Apache Drill TLS security testing..."
    if python3 post_command/drill.py --tls "$HOST" "$PORT"; then
        echo "Apache Drill TLS connection successful"
    else
        echo "Apache Drill TLS connection failed (may not be an Apache Drill service)"
    fi
else
    # Step 4: Apache Drill plain testing
    echo ""
    echo "Step 4: Apache Drill plain connection testing..."
    if python3 post_command/drill.py "$HOST" "$PORT"; then
        echo "Apache Drill plain connection successful"
    else
        echo "Apache Drill plain connection failed (may not be an Apache Drill service)"
    fi
fi

echo ""
echo "=== Apache Drill Security Scan Chain Complete ==="
