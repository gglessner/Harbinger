#!/bin/bash
# Apache Ignite Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Apache Ignite security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Apache Ignite security testing
#
# Always returns 0 for successful chain execution, regardless of final ignite.py result
...
echo "=== Apache Ignite Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe ignite.py can still work
    fi
    
    # Step 4: Apache Ignite TLS testing
    echo ""
    echo "Step 4: Apache Ignite TLS security testing..."
    if python3 post_command/ignite.py --tls "$HOST" "$PORT"; then
        echo "Apache Ignite TLS connection successful"
    else
        echo "Apache Ignite TLS connection failed (may not be an Ignite service)"
    fi
else
    # Step 4: Apache Ignite plain testing
    echo ""
    echo "Step 4: Apache Ignite plain connection testing..."
    if python3 post_command/ignite.py "$HOST" "$PORT"; then
        echo "Apache Ignite plain connection successful"
    else
        echo "Apache Ignite plain connection failed (may not be an Ignite service)"
    fi
fi

echo ""
echo "=== Apache Ignite Security Scan Chain Complete ==="
