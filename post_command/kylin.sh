#!/bin/bash
# Apache Kylin Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Apache Kylin security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Apache Kylin security testing
#
# Always returns 0 for successful chain execution, regardless of final kylin.py result
...
echo "=== Apache Kylin Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe kylin.py can still work
    fi
    
    # Step 4: Apache Kylin TLS testing
    echo ""
    echo "Step 4: Apache Kylin TLS security testing..."
    if python3 post_command/kylin.py --tls "$HOST" "$PORT"; then
        echo "Apache Kylin TLS connection successful"
    else
        echo "Apache Kylin TLS connection failed (may not be an Apache Kylin service)"
    fi
else
    # Step 4: Apache Kylin plain testing
    echo ""
    echo "Step 4: Apache Kylin plain connection testing..."
    if python3 post_command/kylin.py "$HOST" "$PORT"; then
        echo "Apache Kylin plain connection successful"
    else
        echo "Apache Kylin plain connection failed (may not be an Apache Kylin service)"
    fi
fi

echo ""
echo "=== Apache Kylin Security Scan Chain Complete ==="
