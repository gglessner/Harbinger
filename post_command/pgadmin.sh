#!/bin/bash
# pgAdmin Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full pgAdmin security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. pgAdmin security testing
#
# Always returns 0 for successful chain execution, regardless of final pgadmin.py result
...
echo "=== pgAdmin Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe pgadmin.py can still work
    fi
    
    # Step 4: pgAdmin TLS testing
    echo ""
    echo "Step 4: pgAdmin TLS security testing..."
    if python3 post_command/pgadmin.py --tls "$HOST" "$PORT"; then
        echo "✓ pgAdmin TLS connection successful"
    else
        echo "✗ pgAdmin TLS connection failed (may not be a pgAdmin service)"
    fi
else
    # Step 4: pgAdmin plain testing
    echo ""
    echo "Step 4: pgAdmin plain connection testing..."
    if python3 post_command/pgadmin.py "$HOST" "$PORT"; then
        echo "✓ pgAdmin plain connection successful"
    else
        echo "✗ pgAdmin plain connection failed (may not be a pgAdmin service)"
    fi
fi

echo ""
echo "=== pgAdmin Security Scan Chain Complete ==="
