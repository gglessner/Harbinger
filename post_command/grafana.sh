#!/bin/bash
# Grafana Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Grafana security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Grafana security testing
#
# Always returns 0 for successful chain execution, regardless of final grafana.py result
...
echo "=== Grafana Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe grafana.py can still work
    fi
    
    # Step 4: Grafana TLS testing
    echo ""
    echo "Step 4: Grafana TLS security testing..."
    if python3 post_command/grafana.py --tls "$HOST" "$PORT"; then
        echo "Grafana TLS connection successful"
    else
        echo "Grafana TLS connection failed (may not be a Grafana service)"
    fi
else
    # Step 4: Grafana plain testing
    echo ""
    echo "Step 4: Grafana plain connection testing..."
    if python3 post_command/grafana.py "$HOST" "$PORT"; then
        echo "Grafana plain connection successful"
    else
        echo "Grafana plain connection failed (may not be a Grafana service)"
    fi
fi

echo ""
echo "=== Grafana Security Scan Chain Complete ==="
