#!/bin/bash
# Jaeger Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Jaeger security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Jaeger security testing
#
# Always returns 0 for successful chain execution, regardless of final jaeger.py result
...
echo "=== Jaeger Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe jaeger.py can still work
    fi
    
    # Step 4: Jaeger TLS testing
    echo ""
    echo "Step 4: Jaeger TLS security testing..."
    if python3 post_command/jaeger.py --tls "$HOST" "$PORT"; then
        echo "✓ Jaeger TLS connection successful"
    else
        echo "✗ Jaeger TLS connection failed (may not be a Jaeger service)"
    fi
else
    # Step 4: Jaeger plain testing
    echo ""
    echo "Step 4: Jaeger plain connection testing..."
    if python3 post_command/jaeger.py "$HOST" "$PORT"; then
        echo "✓ Jaeger plain connection successful"
    else
        echo "✗ Jaeger plain connection failed (may not be a Jaeger service)"
    fi
fi

echo ""
echo "=== Jaeger Security Scan Chain Complete ==="
