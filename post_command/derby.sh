#!/bin/bash
# Apache Derby Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Apache Derby security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Apache Derby security testing
#
# Always returns 0 for successful chain execution, regardless of final derby.py result
...
echo "=== Apache Derby Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe derby.py can still work
    fi
    
    # Step 4: Apache Derby TLS testing
    echo ""
    echo "Step 4: Apache Derby TLS security testing..."
    if python3 post_command/derby.py --tls "$HOST" "$PORT"; then
        echo "✓ Apache Derby TLS connection successful"
    else
        echo "✗ Apache Derby TLS connection failed (may not be a Derby service)"
    fi
else
    # Step 4: Apache Derby plain testing
    echo ""
    echo "Step 4: Apache Derby plain connection testing..."
    if python3 post_command/derby.py "$HOST" "$PORT"; then
        echo "✓ Apache Derby plain connection successful"
    else
        echo "✗ Apache Derby plain connection failed (may not be a Derby service)"
    fi
fi

echo ""
echo "=== Apache Derby Security Scan Chain Complete ==="
