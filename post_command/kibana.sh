#!/bin/bash
# Kibana Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Kibana security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Kibana security testing
#
# Always returns 0 for successful chain execution, regardless of final kibana.py result
...
echo "=== Kibana Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe kibana.py can still work
    fi
    
    # Step 4: Kibana TLS testing
    echo ""
    echo "Step 4: Kibana TLS security testing..."
    if python3 post_command/kibana.py --tls "$HOST" "$PORT"; then
        echo "✓ Kibana TLS connection successful"
    else
        echo "✗ Kibana TLS connection failed (may not be a Kibana service)"
    fi
else
    # Step 4: Kibana plain testing
    echo ""
    echo "Step 4: Kibana plain connection testing..."
    if python3 post_command/kibana.py "$HOST" "$PORT"; then
        echo "✓ Kibana plain connection successful"
    else
        echo "✗ Kibana plain connection failed (may not be a Kibana service)"
    fi
fi

echo ""
echo "=== Kibana Security Scan Chain Complete ==="
