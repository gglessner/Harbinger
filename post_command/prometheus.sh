#!/bin/bash
# Prometheus Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Prometheus security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Prometheus security testing
#
# Always returns 0 for successful chain execution, regardless of final prometheus.py result
...
echo "=== Prometheus Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe prometheus.py can still work
    fi
    
    # Step 4: Prometheus TLS testing
    echo ""
    echo "Step 4: Prometheus TLS security testing..."
    if python3 post_command/prometheus.py --tls "$HOST" "$PORT"; then
        echo "✓ Prometheus TLS connection successful"
    else
        echo "✗ Prometheus TLS connection failed (may not be a Prometheus service)"
    fi
else
    # Step 4: Prometheus plain testing
    echo ""
    echo "Step 4: Prometheus plain connection testing..."
    if python3 post_command/prometheus.py "$HOST" "$PORT"; then
        echo "✓ Prometheus plain connection successful"
    else
        echo "✗ Prometheus plain connection failed (may not be a Prometheus service)"
    fi
fi

echo ""
echo "=== Prometheus Security Scan Chain Complete ==="
