#!/usr/bin/env python3
"""
IBM Web Administration Tool Secure Admin Console Security Scanner
Author: Garland Glessner <gglessner@gmail.com>
License: GNU GPL

This script tests IBM Web Administration Tool Secure Admin Console connectivity and reports security configuration:
...
def test_ibmwat_auth(host, port, use_ssl=False):
    """Test IBM Web Administration Tool Secure Admin Console authentication"""
    try:
        ...
        try:
            ...
            if 'ibm' in response_data.lower() or 'websphere' in response_data.lower() or 'administration' in response_data.lower():
                ...
                if '401' not in response_data and 'unauthorized' not in response_data.lower():
                    if 'login' not in response_data.lower() or 'password' not in response_data.lower():
                        print(f"IBM Web Administration Tool Secure Admin Console accessible at {host}:{port}")
                        print("VULNERABLE")
                        return True
            ...
            if e.code == 401:
                print(f"IBM Web Administration Tool Secure Admin Console at {host}:{port} requires authentication")
                return False
            elif e.code == 403:
                print(f"IBM Web Administration Tool Secure Admin Console at {host}:{port} - Access forbidden")
                return False
            else:
                print(f"IBM Web Administration Tool Secure Admin Console at {host}:{port} - HTTP {e.code}")
                return False
...
def scan_ibmwat_security(host, port=12105, tls_only=True):
...
def main():
    parser = argparse.ArgumentParser(description='IBM Web Administration Tool Secure Admin Console Security Scanner')
    parser.add_argument('host', help='IBM Web Administration Tool host to test')
    parser.add_argument('port', nargs='?', type=int, default=12105, help='IBM Web Administration Tool port (default: 12105)')
    ...
    scan_ibmwat_security(host, port, tls_only=True)
