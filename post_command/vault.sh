#!/bin/bash
# Vault Security Scanner Chain
# Author: Garland Glessner <gglessner@gmail.com>
# License: GNU GPL
#
# Runs the full Vault security scanning chain:
# 1. Port check
# 2. TLS detection  
# 3. Certificate collection
# 4. Vault security testing
#
# Always returns 0 for successful chain execution, regardless of final vault.py result
...
echo "=== Vault Security Scan Chain for $HOST:$PORT ==="
...
if [ "$TLS_DETECTED" = true ]; then
    ...
    if python3 post_command/cert_collector.py "$HOST" "$PORT"; then
        ...
        # Continue anyway - maybe vault.py can still work
    fi
    
    # Step 4: Vault TLS testing
    echo ""
    echo "Step 4: Vault TLS security testing..."
    if python3 post_command/vault.py --tls "$HOST" "$PORT"; then
        echo "✓ Vault TLS connection successful"
    else
        echo "✗ Vault TLS connection failed (may not be a Vault service)"
    fi
else
    # Step 4: Vault plain testing
    echo ""
    echo "Step 4: Vault plain connection testing..."
    if python3 post_command/vault.py "$HOST" "$PORT"; then
        echo "✓ Vault plain connection successful"
    else
        echo "✗ Vault plain connection failed (may not be a Vault service)"
    fi
fi

echo ""
echo "=== Vault Security Scan Chain Complete ==="
